...
- 2026-06-11: UI overhaul — declutter command center, branching tech tree, narrative intro.
  - Task 1 (Declutter): Removed vestigial `createGeopoliticalMap` section from ExpandedCommandCenter (the mini-map duplicated the canvas world map). Removed associated helpers (MapPoint, FACTION_MAP_POINTS, tensionKey, getPairTension, areAllied, createSvgElement import) and dead `createRecentLog` function. Removed ~90 lines of `.command-center__geo-*` CSS.
  - Task 2 (Tech Tree — data): Added 7 new capability nodes to create three distinct lab sub-paths: (A) Scaling — long_context → compute_optimal_training → world_models; (B) Agents — tool_use/function_calling → unreliable_agent → coding_automation → reliable_agent (simplified prereqs) → agent_swarms (now also requires reliable_agent); (C) Reasoning — chain_of_thought → inference_time_compute → extended_reasoning (prereq changed from reliable_agent to inference_time_compute). New nodes: cap_compute_optimal_training, cap_inference_time_compute, cap_multimodal_agents, cap_self_critique. Added 4 new safety nodes creating clearer interpretability/alignment/governance sub-paths: safe_staged_deployment (governance), safe_value_alignment (alignment), safe_feature_steering (interpretability capstone). Updated safe_scalable_oversight to require safe_feature_steering (interpretability) instead of safe_circuit_analysis, making three converging paths explicit.
  - Task 2 (Tech Tree — UI): Rewrote BranchScreen.renderBranchScreen to render a left-to-right tier graph with absolutely-positioned nodes and SVG bezier-curve connectors (branch-tree__canvas, branch-tree__svg, branch-tree__edge). Nodes are laid out with calcGridLayout (sorts within each tier by parent row position to minimize crossing). Connector colors: active (both ends unlocked) = branch color; available (parent unlocked) = dashed branch color; locked = dashed line. Added tier labels row above the canvas. Added new CSS .branch-tree__* rules to styles.css.
  - Task 3 (Narrative Intro): Rewrote IntroSequence INTRO_STEPS from a mechanics-dump 5-step briefing to 4 narrative beats (The Year is 2026 / The Threshold / Five Factions One Race / Director) plus an optional 5th mechanics beat. Added FACTION_DESCRIPTIONS mapping with 1-2 sentence descriptions per faction (identity, playstyle, special ability, victory path) and rendered them in the faction selection screen above the stat bars.
- Validation (2026-06-11):
  - `npx tsc --noEmit` → passes (no new errors; all pre-existing errors are in src/ai/* and src/cli.ts, not in changed files)
  - `npm run build` → pass (vite build, 71 modules, 883ms)
  - `npm test -- --run` → 274 passed, 12 failed (same pre-existing heuristic.test.ts failures, no new failures)
  - `npx playwright install chromium` → FAILED (network download blocked in environment); E2E tests not runnable; noted in PR body
